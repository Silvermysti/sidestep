<script>
  ...
  const THEMES = {
    ...
    autumn: { label: 'Autumn', bg: 'url(/scene/autumn-bg.png)', grass: '/scene/autumn-grass.png', tile: 1459, dot: '#d5852f', pos: 'center 90%' },
...
<main class:focus={isFocus} class:break={!isFocus}>
  ...
  {#if t && s && l}
    ...
    {#if tab === 'focus'}
      ...
      <div class="habitat" style="--habitat-bg: {scene.bg}; --habitat-size: {scene.zoom ?? 'cover'}; --habitat-pos: {scene.pos ?? 'center'}; --grass-img: url({scene.grass}); --grass-tile: {scene.tile}px">
        <div class="hud">
